<?php
/* @var $this UserController */
/* @var $data User */
?>

<div class="view">

	<b><?php echo CHtml::encode($data->getAttributeLabel('uid')); ?>:</b>
	<?php echo CHtml::link(CHtml::encode($data->uid), array('view', 'id'=>$data->uid)); ?>
	<br />

        <!--
	<b><?php echo CHtml::encode($data->getAttributeLabel('enabled')); ?>:</b>
	<?php echo CHtml::encode($data->enabled); ?>
	<br />
        -->

	<b><?php echo CHtml::encode($data->getAttributeLabel('loginname')); ?>:</b>
	<?php echo CHtml::encode($data->loginname); ?>
	<br />

	<b><?php echo CHtml::encode($data->getAttributeLabel('familyname')); ?>:</b>
	<?php echo CHtml::encode($data->familyname); ?>
	<br />

	<b><?php echo CHtml::encode($data->getAttributeLabel('firstname')); ?>:</b>
	<?php echo CHtml::encode($data->firstname); ?>
	<br />

        <!--
	<b><?php echo CHtml::encode($data->getAttributeLabel('password')); ?>:</b>
	<?php echo CHtml::encode($data->password); ?>
	<br />
        -->

	<b><?php echo CHtml::encode($data->getAttributeLabel('club_id')); ?>:</b>
	<?php
        
        //echo CHtml::encode($data->club_id);
            $golf_club = GolfClub::model()->findByPk($data->club_id);
            $golf_club_name = isset($golf_club->gc_name)?$golf_club->gc_name:'*';
            echo CHtml::encode($golf_club_name);
        ?>
	<br />

	<?php /*
	<b><?php echo CHtml::encode($data->getAttributeLabel('deleted')); ?>:</b>
	<?php echo CHtml::encode($data->deleted); ?>
	<br />

	*/ ?>


</div>